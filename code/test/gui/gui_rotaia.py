...
class UniversalDobotGUI:
    def __init__(self, root):
        ...
        self.root.geometry("750x660") # Aumentata l'altezza per ospitare il nuovo controllo
    ...
    def setup_styles(self):
        ...
        style.configure("Conveyor.TCheckbutton", background="#2C313C", foreground="white", font=("Arial", 10, "bold"))
    ...
    def create_widgets(self):
        ...
        # ==========================================
        # NEW PANEL: Controlli Nastro Trasportatore (Esclusivo Magician)
        # ==========================================
        self.conveyor_frame = ttk.LabelFrame(self.root, text=" Nastro Trasportatore (Conveyor Belt) ", padding=15)
        ...
        self.chk_conveyor = ttk.Checkbutton(self.conveyor_frame, text="Attiva Nastro", variable=self.conv_enabled_var, 
                                            style="Conveyor.TCheckbutton", command=self.toggle_conveyor_widgets)
        ...
        # Stato iniziale disabilitato dello slider e del bottone stop
    ...
    # ==========================================
    # LOGICA DI CONTROLLO DINAMICA E DI INTERFACCIA
    # ==========================================
    def get_api(self):
    ...
    def on_device_change(self, event):
        ...
        # Mostra/Nascondi dinamicamente il pannello del nastro trasportatore
        if self.device_var.get() == "Magician":
            # Inserisce il frame del nastro sopra a quello della ventosa
            self.suction_frame.pack_forget()
            self.conveyor_frame.pack(fill="x", padx=20, pady=5)
            self.suction_frame.pack(fill="x", padx=20, pady=15)
        else:
            # Rimuove il nastro dall'interfaccia se è selezionato Magician Lite
            self.stop_conveyor()
    ...
    def toggle_conveyor_widgets(self):
        """Attiva o disattiva gli slider del nastro in base alla checkbox."""
    ...
    def on_conveyor_speed_change(self, val):
        ...
        if self.is_connected and self.device_var.get() == "Magician" and self.conv_enabled_var.get():
            try:
                # Esegue set_converyor (come definito nella libreria MagicianApi)
                # index=0 (primo nastro), enable=True, speed=valore_dello_slider
                api_magician.set_converyor(port_name=self.port, index=0, enable=True, speed=speed_val, is_queued=False)
            except Exception as e:
                print(f"Errore controllo nastro: {e}")

    def stop_conveyor(self):
        """Ferma il nastro impostando la velocità a 0 e ripristinando lo slider."""
        self.conv_speed_scale.set(0)
        self.lbl_conv_speed.config(text="Velocità: 0")
        if self.is_connected and self.device_var.get() == "Magician":
            try:
                # Ferma impostando enable=False e velocità=0
                api_magician.set_converyor(port_name=self.port, index=0, enable=False, speed=0, is_queued=False)
                print("Nastro Trasportatore: ARRESTATO")
            except Exception as e:
                print(f"Errore arresto nastro: {e}")
