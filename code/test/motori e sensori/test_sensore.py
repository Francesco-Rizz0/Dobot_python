...
def main():
    ...
    dobotEdu.magician.set_infrared_sensor(port_name=port_name,port=2,enable=True,version=1,is_queued=True)
    continuare = True
    ...
    while continuare:
        nigga = dobotEdu.magician.get_infrared_sensor(port_name=port_name,port=2,is_queued=True)
        print(nigga)
        if(nigga["status"]==1):
            continuare = False
